...
from https://github.com/HumanCompatibleAI/imitation/blob/master/src/imitation/rewards/reward_wrapper.py
"""
...
from sb3based.rnnbc import pack_structure
...
class RewardVecEnvWrapper(vec_env.VecEnvWrapper):
    ...
    def step_wait(self):
        ...
        for i in range(len(dones)):
            single_obs = tree.map_structure(lambda x: x[i], obs)
            single_done = dones[i]
            single_infos = infos[i]
            if single_done:
                single_obs = single_infos["terminal_observation"]
        
            obs_fixed.append(single_obs)
        obs_fixed = pack_structure(obs_fixed)

        rews = self.reward_fn(self._old_obs, self._actions, obs_fixed, np.array(dones))
        assert len(rews) == len(dones), "must return one rew for each env"
